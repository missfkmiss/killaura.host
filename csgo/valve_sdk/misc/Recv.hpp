...
private:
	...
	RecvProp* m_pTarget;
	RecvVarProxyFn m_pOriginal;

public:

	RecvPropHook( RecvProp* target_property )
	{
		this->m_pTarget = target_property;
		this->m_pOriginal = target_property->m_ProxyFn;
	}

	~RecvPropHook( )
	{
        m_pTarget->m_ProxyFn = this->m_pOriginal;
	}

	RecvVarProxyFn GetOriginalFunction( )
	{
		return this->m_pOriginal;
	}

	void SetProxyFunction( RecvVarProxyFn user_proxy_fn )
	{
        m_pTarget->m_ProxyFn = user_proxy_fn;
	}
};
