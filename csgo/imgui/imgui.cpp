...
/*
 ...
 - Limited layout Cfgs, intricate layouts are typically crafted in code
 ...
 - Call and read ImGui::ShowDemoWindow() for demo code demonstrating most Cfgs.
 ...
 - Init: retrieve the ImGuiIO structure with ImGui::GetIO() and fill the fields marked 'Settings': at minimum you need to set io.DisplaySize
   (application resolution). Later on you will fill your keyboard mapping, clipboard handlers, and other advanced Cfgs but for a basic 
 ...
 - Every frame:
    ...
    - Call ImGui::Render() as late as you can to end the frame and finalize render data. it will call your io.RenderDrawListFn handler.
       (Even if you don't render, call Render() and ignore the callback, or call EndFrame() instead. Otherwhise some Cfgs will break)
...
ImGuiStyle::ImGuiStyle()
{
  ...
  // ScrollbarRounding       = 9.0f;             // Radius of grab corners rounding for scrollbar
	...
	ScrollbarSize = 1;            // Width of the vertical scrollbar, Height of the horizontal scrollbar
...
{
    ImGuiContext& g = *GImGui;
    //if (!g.IO.NavActive)  // [2017/10/06] Removed this possibly redundant test but I am not sure of all the side-effects yet. Some of the Cfg here will need to work regardless of using a _NoNavInputs flag.
    //    return;
...
void ImGui::NewFrame()
{
    ...
    // Calculate frame-rate for the user, as a purely luxurious Cfg
...
{
    // Old API Cfg: we could pass the initial window size as a parameter. This was misleading because it only had an effect if the window didn't have data in the .ini file.
    if (size_first_use.x != 0.0f || size_first_use.y != 0.0f)
        ImGui::SetNextWindowSize(size_first_use, ImGuiCond_FirstUseEver);

    // Old API Cfg: override the window background alpha with a parameter.
...
bool ImGui::MyButtonEx(const char* label, const char* icon, ImFont* icon_font, bool inuse, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	if (inuse)
	{
		...
		// lines
		RenderFrame(ImVec2(bb.Min.x - 25, bb.Min.y + 1), ImVec2(bb.Max.x, bb.Min.y), ImColor(87, 48, 161, 255));
		RenderFrame(ImVec2(bb.Min.x - 25, bb.Max.y - 1), ImVec2(bb.Max.x, bb.Max.y), ImColor(87, 48, 161, 255));
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (display_frame)
    {
        ...
        if (g.LogEnabled)
        {
            // NB: '##' is normally used to hide text (as a library-wide Cfg), so we need to specify the text range to make sure the ## aren't stripped out here.
