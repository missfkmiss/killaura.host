...
{
    // Simple word-wrapping for English, not full-Cfgd. Please submit failing cases!
...
ImVec2 ImFont::CalcTextSizeA(float size, float max_width, float wrap_width, const char* text_begin, const char* text_end, const char** remaining) const
{
    ...
    while (s < text_end)
    {
        ...
        {
            // Calculate how far we can render. Requires two passes on the string data but keeps the code simple and not intrusive for what's essentially an uncommon Cfg.
            if (!word_wrap_eol)
            {
